...
export default {
    ...
    props: {
         ...
         * @type {String}
         *
         * 여기 label은 q-field 의 label 이 아니며, (여기의 label은 'name' props가 담당한다.)
         * options 가 객체로 주워졌을 때, key 가 되는 프로퍼티의 이름을 의미한다.
         * (여종훈)
         */
        label: {
            type: String,
    ...
    computed: {
            ...
            return {
                search: {
                    attributes: {
                        ...
                        readonly: !this.searchable || this.readonly, // searchable 과 readonly 는 동일함 (여종훈)
                        id: this.inputId,
                        ...
                        ...(this.dropdownOpen && this.filteredOptions[this.typeAheadPointer]
                            ...
                            : {}),
                        autofocus: this.autofocus, // 여종훈(작동안함)
                    },
                    events: {
         ...
         */
        searchPlaceholder() {
            return this.isValueEmpty && this.placeholder ? this.placeholder : undefined
        },
         ...
         */
        showClearButton() {
            return !this.multiple && this.clearable && !this.open && !this.isValueEmpty && !this.noCloseIcon // noCloseIcon은 여종훈 추가
        },
         ...
         */
        renderHint() {
            if (!this.scope.noOptions.searching && this.selectedValue.length === 0) {
                return this.searchingHint
            }
            if (this.filteredOptions.length === 0) {
                return '<span class="text-deep-orange">' + this.noDataHint + '</span>'
            }
            return this.etcHint
        },
        renderErrorMessage() {
            ...
            if (this.filteredOptions.length === 0) {
                return this.noDataErrorMessage
            }
            return this.etcErrorMessage
        },
    },
    ...
    methods: {
        ...
        toggleDropdown(event) {
            ...
            const ignoredButtons = [...(this.deselectButtons || []), ...([this.$refs['clearButton']] || [])]

            if (
                this.searchEl === undefined ||
                ignoredButtons.filter(Boolean).some(ref => ref.contains(event.target) || ref === event.target)
            ) {
                event.preventDefault()
                return
            }
    ...
    <div :dir="dir" class="v-select" :class="stateClasses">
        ...
        >
            ...
            <div ref="selectedOptions" class="vs__selected-options">
                <q-field
                    ...
                    class="vs__search"
                    :class="{
                        'q-field--float': selectedValue.length > 0 || !!scope.search.attributes.value,
                    }"
                    :error="$props.error"
                >
                    <template v-slot:control>
                        ...
                        <slot name="search" v-bind="scope.search">
                            <input v-bind="scope.search.attributes" v-on="scope.search.events" class="vs__search" />
                        </slot>
                    </template>
                    <template v-slot:append>
                        ...
                        <div ref="actions" class="vs__actions">
                            ...
                            >
                                <q-icon name="close" color="grey" size="1.3rem" />
                            </button>
                            ...
                            <slot name="spinner" v-bind="scope.spinner">
                                <div v-show="mutableLoading" class="vs__spinner">검색 중...</div>
                            </slot>
                        </div>
                    </template>
                    <template v-slot:hint>
                        <div style="min-height: 12px" v-html="renderHint" />
                    </template>
                    <template v-slot:error>
                        <div style="min-height: 12px">{{ renderErrorMessage }}</div>
                    </template>
        ...
        <transition :name="transition">
            <ul
                ...
                @mouseup="onMouseUp"
                :style="{ marginTop: hideBottomSpace ? '2px' : '-18px' }"
            >
                <!--:style="{ marginTop: hideBottomSpace ? '2px' : '-18px' }"-->
